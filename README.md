...
2. **テナント（棟）＋チーム** — チームより上位に棟がある。主鍵で自棟、招待鍵で別棟、共通鍵で共有棟。主所属は1つ、兼務・プロジェクトは追加タグ。親チーム所属者は配下を閲覧できる。AI アプリ・RAG ナレッジ・保存プロンプトの共有は **活性棟の所属チーム** を軸に制御
...
- **棟**: `tenants` がチームより上位。既存データはデフォルト棟、共通チームは共有棟
