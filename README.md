...
> **バージョン:** 現在 **v0.11.0**（マルチテナントでナレッジ・アプリ・履歴を分ける。おすすめアプリ、Markdown エディタの HTML/PPTX 書き出し）。v0.10.0 = ノートブック＝資料を集めて調べ項目に整理。スキル/MCP/OCR。Web SSH、アプリ表示の編集、ヒアリングシート、v0.9.0 = 提案実装 第3弾：Markdown エディタ＋オプションの各種文書作成（生成 API はプラガブル）、v0.8.0 = 提案実装 第2弾：オンラインフォーム（patchform）／手続き（申請束）・申請受付／マイ手続き（docmaker）／手続き MCP、v0.7.0 = 提案実装（書類読取とチェック・日程調整・様式 Excel 文書生成）、v0.6.0 = 添付・ナレッジの個人情報検知／ナレッジ専用ページ／チャット大容量添付の mapreduce／Dify エラー分類、v0.5.0 = 本番静的ビルド/デプロイ整備・複数LLM/埋め込み/画像モデルの差し替え・SAML/認証堅牢化、v0.4.0 = 構造化 RAG・ナレッジ MCP・Dify 連携事例・マイナンバー検査、v0.3.x = 出典表示・画像生成一本化等、v0.2.x = 自治体・閉域向け拡張、v0.1.0 = ローカル源内化の第一段階。
...
> **変更履歴:** [CHANGELOG.md](CHANGELOG.md)（**v0.11.0** = マルチテナント・おすすめアプリ・Markdown エディタの HTML/PPTX 書き出し、**v0.10.0** = ノートブック（調べて項目に整理・スキル/MCP/OCR）・Web SSH・アプリ表示の編集、**v0.9.0** = 提案実装 第3弾：Markdown エディタ＋オプションの各種文書作成（生成 API はプラガブル・契約は OSS 公開）、**v0.8.0** = 提案実装 第2弾：オンラインフォーム（patchform）・手続き（申請束）・申請受付・マイ手続き（docmaker）・手続き MCP、**v0.7.0** = 提案実装：書類読取とチェック・日程調整・様式 Excel 文書生成、**v0.6.0** = PII 検知・ナレッジ専用ページ・大容量添付 mapreduce 等、**v0.5.0** = 本番静的ビルド/デプロイ整備・複数LLM/埋め込み/画像モデル差し替え・SAML/認証堅牢化、**v0.4.0** = 構造化 RAG・ナレッジ MCP・Dify 連携事例・マイナンバー検査、それ以前は CHANGELOG 参照）
...
1. **自治体実務を想定した機能** — 監査ログ（3 年以上保持）、利用者 CSV 一括管理、モデル利用制御、禁止語／個人情報検知（添付警告・ナレッジラベル、任意の GiNZA NER）、プロンプトテンプレート、ノートブック（資料を構造化して項目・対話・ヒアリングシート。標準起動）、日程調整（オプション profile）、書類領域分割チェック（オプション profile）、オンライン申請フォーム・手続き（申請束）・マイ手続き（オプション profile）、Markdown エディタ＋各種文書作成（調達仕様書・セキュリティポリシー/実施手順・業務マニュアル・アクションプラン等を AI 対話で。生成 API はプラガブル、オプション profile）、Web SSH 端末（オプション profile）、契約終了時の完全削除と報告書生成
2. **テナント（棟）＋チーム** — チームより上位に棟がある。主鍵で自棟、招待鍵で別棟、共通鍵で共有棟。主所属は1つ、兼務・プロジェクトは追加タグ。親チーム所属者は配下を閲覧できる。AI アプリ・RAG ナレッジ・チャット履歴・保存プロンプトは **活性棟** で分ける
...
- CSV 列: `action`, `username`, `email`, `name`, `password`, `groups`, `enabled`, `tenant`（棟）等
- **棟（`tenant`）**: 棟名か棟 ID を書く。**新規登録は必須**（共有棟は招待で付けるため指定不可）。Keycloak 反映のあと backend が主鍵として付与する
- 1 名だけなら「利用者登録」タブのフォーム（棟は必須の選択）でも作れる
...
| **新規利用者** | 利用者一括管理 exApp（利用者登録／CSV。棟を必須指定） | 実運用の利用者は棟を指定して登録し、個別パスワードを発行。Keycloak 管理コンソールだけで作った人は棟に入らないため、招待か CSV で棟を付ける |
...
1. AI アプリ実行後、結果画面の「ファイル一覧」に **ファイル名のみ**が表示されます（生 URL は非表示）。
   Markdown エディタの「書き出し」も同じ配信方式です（`carrier` ではリンクファイル、`open` では zip の直接ダウンロード）。
...
- **棟**: `tenants` がチームより上位。既存データはデフォルト棟。共通チームはデフォルト棟の部屋。共有棟の鍵は自動では付与しない
- **登録は棟が先**: 真のマルチテナント運用では、利用者は必ずどれかの棟の一員として登録する（利用者登録／CSV で棟を必須指定）。ログインだけでは棟を自動付与せず、どの棟の鍵も無い利用者はカタログ・履歴が空になる（画面に案内を出す）。デフォルト棟は既存データの移行寄せで、単一組織運用でも使える普通の組織棟として残す
- **履歴など**: チャット履歴・保存プロンプト・ピン留め・AI アプリ実行履歴は活性棟ごと。ナレッジを使う処理が他棟へ混ざらないようにする。既存行はデフォルト棟へ移す
- **鍵**: 主鍵（自棟）／招待鍵（別棟）／共通鍵（共有棟）。共有棟の鍵は自動では付与しない。別組織または共有棟の鍵がある人だけヘッダーで切り替える。スーパー管理者も棟が1つなら切り替えは出さない（「棟の管理」で操作する）
- **主所属 + 追加タグ**: 主所属は1つ（`team_users.isPrimary`）。兼務・プロジェクトは追加の明示所属
- **親子（任意）**: `teams.parentTeamId`。同じ棟の中だけ。親所属者は配下課のアプリ・ナレッジを閲覧できる（書込・管理は明示メンバーのみ）
- **複数所属**: 1 人の利用者が **複数チーム** に所属可能（`team_users` の `(teamId, userId)` 複合キー）
- **可視範囲**: 利用者は **活性棟の所属チーム + 共有棟** の公開 AI アプリを利用可能。棟ごとの `features` でカタログを隠せる
- **保存プロンプトの共有**: 作成時に「全体公開」または **所属チーム（複数選択）** を指定可能。
  チーム ID が共有タグとして保存され、所属メンバーが利用できる

### 権限

- 棟の作成/招待/機能出し分け（ヘッダー右上「アカウント」→「棟の管理」）
- おすすめアプリ（トップ／サイド）はシステム管理者が全体で設定（「アカウント」→「おすすめアプリ」）。AIアプリ一覧は対象外。利用者向け一覧は未起動を隠す。管理画面では公式アプリの存在と起動状態を出す
- ナレッジ管理はおすすめ（専用ページ）。ナレッジ検索はおすすめ設定と AI アプリ一覧。棟の機能でもそれぞれ出し分けできる
- チーム・メンバー・AI アプリの作成/編集/削除（ヘッダー右上「アカウント」→「チーム管理」）
- 権限モデル:
  - **スーパー管理者**（`SystemAdminGroup`）: 全棟を見る。棟の作成/削除、チーム削除、管理者ツール。人数は最少（一人でも可）
  - **棟の管理者**（`tenant_memberships.isAdmin`）: その棟の招待・機能出し分け・チーム作成・ナレッジ/アプリ/メンバー。他棟は見えない
  - チーム管理者（`team_users.isAdmin`）: 自チームのメンバー/アプリを管理。**ログイン時に自動で `TeamAdminGroup` が付与**され、Keycloak 側でのグループ手動設定は不要
  - 一般メンバー: 活性棟の所属チームのアプリを閲覧・実行。棟は意識しない
- デフォルト棟の共通チーム（`00000000-0000-0000-0000-000000000000`）は自組織の共通ナレッジ／カタログ。共有棟は招待した人だけが見る
