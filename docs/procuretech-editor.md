...
（見出し骨格＋節ごとの用途・役割と要点 → 伏せたレビュー。題名は事実＋含意の完全文）。スライドは DADS のパワポ応用（余白・型スケール・
セマンティック色）と、数値ページではダッシュボードガイドの左上→右下を使う。
分岐・関係・完成イメージは図枠を置く。原稿の ```` ```mermaid ```` は書き出し前に 1 回 PNG 化して埋め、ノートが新たに図にした枚だけ合成中に Mermaid→PNG する。画像化できなければ flowchart を箱と矢印で描く。
スライド題名と同じ見出しは本文に出さない。文書番号・日付など長い値は大きな KPI にせず項目表にする。根拠原文は pptx のスピーカーノートに残す。
