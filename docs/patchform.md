...
手続きの作成は手作業が原則です。手引きや庁内マニュアルのファイルがあれば、未公開の案内・様式・手続きの第1版を一度に作れます（本文の貼り付けはしません）。読める形式は txt / md / pdf / docx / xlsx / pptx / xls です（rag-app と同じ抽出）。古い doc / ppt は取れる範囲だけ読みます。スキャン画像だけの PDF は読めません。例規からは作りません。文書に無い分岐は「【確認】」として手続きの説明に残します。自動公開はしません。
...
各様式の下書き・提出・取下げは今までどおりフォーム単位です。束は進捗を見るだけです。持ち物の文言が添付台紙のチェックボックスと一致するときは、空欄なら自動で付けます。下書きがある欄は触りません。
