...
を同梱しています。ナビゲーションシート
（`B1=hearing-sheet`、設問と回答は別セル、`生成指示`）を読み取り、
...
- 汎用リファレンス実装は先頭シートの種別マーカーと設問行を読む。
  - `B1`: 種別マーカー `hearing-sheet`（旧 `navigation-sheet` も可）
  - `A2` / `B2`: 見出し `項目` / `回答`（旧見出し `値` も読む）
  - 3 行目以降: 設問（A）と回答（B）。各行が 1 つの材料ファイル。回答の改行はセル内改行
  - A 列 `生成指示` の B セル: 成果物 Markdown を書くための処理指示（LLM）
  - 旧形式（A 列 `ナビゲーション` の B に Markdown 表）も読む
  項目の追加・改名・削除は自由。空雛形は `GET /template/hearing`（`navigation` も可）。
  画面からの作成は [`docs/notebook.md`](notebook.md)。
  結果 zip は `README.md`（概要と同梱一覧）、`01_設問名.md`（回答の転記。表・見出しなど Markdown はそのまま）、
...
- 生成側は連結した Markdown を指定形式へ変換する。
  ...
  - **html / pptx / txt / md**: 公開リファレンス実装 `procuretech-generate-app` が担う。
    ...
    見出し骨格のうえに節ごとの用途・役割と要点を載せ、題名は原文の事実（と判断）を完全文にし、
    伏せた内容レビューで deck JSON を組み、決定論レンダラ
