...
- Whisper に `WHISPER_DEVICE` を追加（既定は CPU）。画像生成・文字起こしの本体は環境側で差し替え（`SD_API_URL` / `WHISPER_APP_URL`）。`docker-compose.spark.yml` があれば本番 compose が任意 include する。構築メモは `docs/spark.md`
