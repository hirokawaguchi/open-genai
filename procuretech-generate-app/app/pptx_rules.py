...
- タイトルはページの役割で選ぶ（テーマ／問いかけ／結論）。見出しのコピーにしない。「です」「ます」で終えない。
- タイトルだけ通し読みして 1 本の話になること。ラベル前置き（「現状：」）禁止。
- 「3つの理由」のような中身の個数をタイトルに入れない。
- 1 枚 1 メッセージ。溢れたら分割。原文に無い数値・固有名を作らない。
- 見せ方は先に基本形: fullwidth-points / axis-table / premise-conclusion / before-after-split / chart-insight。手順だけ chevron-steps。カード羅列は最後。
